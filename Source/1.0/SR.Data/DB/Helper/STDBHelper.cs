...
namespace SR.Data.DB.Helper
{
    public abstract class STDBHelper : ADBHelper
    {
        ...
        public override ConvertibleObject GetResult(ISQLPlan plan)
        {
            return new ConvertibleObject(__CallPreparedCommand(plan, __GetResultCMDUsing));
        }

        public override int Execute(ISQLPlan plan)
        {
            return Converts.objectToInt(__CallPreparedCommand(plan, __ExecuteCMDUsing));
        }
